...
from wavesim.utilities import normalize, pad_boundaries, add_absorbing_boundaries
...
fc1 = (
    (torch.fft.ifftn(coordinates_f_sq(n_size) * torch.fft.fftn(side))).real.cpu().numpy()
)  # discard tiny imaginary part due to numerical errors
...
fc2 = (torch.fft.ifftn(coordinates_f_sq(n_size // 2) * torch.fft.fftn(side2))).real.cpu().numpy()
