...
if os.path.exists(filename):
    ...
    domain_ref = Helmholtz(permittivity=n, periodic=periodic, wavelength=wavelength, pixel_size=pixel_size)

    start_ref = time()
    # Field u and state object with information about the run
    u_ref, iterations_ref, residual_norm_ref = preconditioned_richardson(domain_ref, source, max_iterations=10000)
    ...
    u_ref = u_ref[*(slice(boundary_widths[i], u_ref.shape[i] - boundary_widths[i]) for i in range(3))].cpu().numpy()
    ...
    for n_boundary in corrs:
        ...
        start_n = time()
        u_n, iterations_n, residual_norm_n = preconditioned_richardson(domain_n, source, max_iterations=10000)
        sim_time_n = time() - start_n
        print(f"\nTime {sim_time_n:2.2f} s; Iterations {iterations_n}; Residual norm {residual_norm_n:.3e}")
        # crop the field to the region of interest
        u_n = u_n[*(slice(boundary_widths[i], u_n.shape[i] - boundary_widths[i]) for i in range(3))].cpu().numpy()
    ...
    np.savez_compressed(filename, corrs=corrs, sim_time=sim_time, iterations=iterations, ure_list=ure_list)
...
ax0 = axs[0]
ax0.semilogy(x, ure_list[:length], "r", lw=1.0, marker="x", markersize=3)
...
ax1 = axs[1]
start = 4
ax1.plot(x[start:], iterations[start:length], "g", lw=1.0, marker="+", markersize=3)
...
ax2 = axs[2]
ax2.plot(x[start:], sim_time[start:length], "b", lw=1.0, marker="*", markersize=3)
