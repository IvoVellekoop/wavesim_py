...
def test_2DHighContrast(n_domains):
    ...
    max_iterations = int(1.e+4)  # 1.e+4 iterations gives relative error 1.65e-4 with the matlab test result, but takes ~140s
    wavelength = 0.532
    ppw = 3*np.max(abs(n_contrast+1))

    anysim2D_HC = AnySim(wavelength=wavelength, ppw=ppw, boundary_widths=boundary_widths, n=n, source=source, n_domains=n_domains, overlap=boundary_widths, max_iterations=max_iterations)
