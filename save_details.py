...
class LogPlot:
    ...
    def plot_details(self):
        ...
        if self.base.n_dims == 1:
            ...
            for z_slice in [0, int(self.u_computed.shape[2] / 2), int(self.u_computed.shape[2] - 1)]:
                ...
                self.plot_field_iters(z_slice)         # movie/animation/GIF
        plt.close('all')
        print('Plotting done.')
    ...
    def plot_field_n_residual(self):
        ...
        plt.subplot(2, 1, 2)
        res_plots = plt.loglog(np.arange(1, self.state.iterations + 2),
        ...
        plt.loglog(np.arange(1, self.state.iterations+2), self.state.full_residuals, lw=3., c='k',
    ...
    def plot_field_iters(self, z_slice=0):  # movie/animation/GIF
        """ Plot an animation of the field wrt iterations and save as mp4 """
        u_iter = np.real(self.state.u_iter)

        if self.base.n_dims == 3:
            u_iter = u_iter[:, :, :, z_slice]
            if hasattr(self, 'u_reference'):
        ...
        if self.base.n_dims == 1:
            ...
            plot_data = plt.imshow(u_iter[0], cmap='seismic', vmin=-v_lim, vmax=v_lim, animated=True)
            plt.colorbar()
        title = plt.title('')
        ...
        if self.state.iterations > 100:
            plot_iters = 100
            iters_trunc = np.repeat(np.linspace(0, self.state.iterations - 1, plot_iters).astype(int),
            ...
            iters_trunc = np.repeat(np.arange(self.state.iterations), self.base.total_domains)
        ...
        def animate(i):
            if self.base.n_dims == 1:
                ...
                plot_data.set_array(u_iter_trunc[i])  # update the data.
    ...
    def image_field_n_residual(self, z_slice=0):  # png
        ...
        if hasattr(self, 'u_reference'):
            plt.subplot(2, 2, 3)
            plt.imshow(u_reference, cmap='seismic', vmin=-v_lim, vmax=v_lim)
            plt.colorbar(shrink=shrink, pad=pad)
            plt.title(self.label)

            plt.subplot(2, 2, 4)
            plt.imshow(u_reference - u, cmap='seismic')
            plt.colorbar(shrink=shrink, pad=pad)
