...
class MaxwellDomain(Domain):
    ...
    def __init__(self,
        ...
        self._x = [torch.cat((permittivity, torch.zeros_like(permittivity), torch.zeros_like(permittivity)), -1)] + [torch.tile(torch.zeros_like(permittivity), (1, 1, 1, 3)) for _ in range(n_slots - 1)]
    ...
    def propagator(self, slot_in: int, slot_out: int):
        ...
        if self.active:
            for c in range(3):
                torch.fft.fftn(self._x[slot_in][..., c], out=self._x[slot_out][..., c])

            div = self.scale * sum(self._f[i] * self._x[slot_out][..., i] for i in range(3))/-(self.scale*self.shift+1.0)

            for c in range(3):
                self._x[slot_out][..., c].sub_(self._f[c].mul(div))
                self._x[slot_out][..., c].mul_(self.propagator_kernel)

            for c in range(3):
                torch.fft.ifftn(self._x[slot_out][..., c], out=self._x[slot_out][..., c])
...
def _make_wrap_matrix(L_kernel, n_boundary):
    ...
    :return: the matrix for the wrapping correction
