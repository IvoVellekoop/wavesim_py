...
from wavesim.iteration import preconditioned_richardson  # to run the anysim iteration
...
if os.path.exists(filename):
    ...
    for nx, ny in product(domains, domains):
        ...
        u, iterations, residual_norm = preconditioned_richardson(
            domain, source, max_iterations=10000
        ...
        # %% Save data to file
        data = (
            f"Size {n_size}; Boundaries {boundary_widths}; Domains {n_domains}; "
...
num_domains = [
    (int(num_domains[i].split(",", maxsplit=1)[0]), int(num_domains[i].split(",", maxsplit=1)[-1]))
    for i in range(len(num_domains))
]
