...
from wavesim.utilities import pad_boundaries, add_absorbing_boundaries
...
def test_1d_glass_plate(n_domains, periodic):
    ...
    n, boundary_array = add_absorbing_boundaries(
        n**2, boundary_widths
    ...
    if n_domains is None:  # 1-domain, periodic boundaries (without wrapping correction)
        domain = Helmholtz(permittivity=n, periodic=periodic, wavelength=wavelength)
    ...
    u_computed = preconditioned_richardson(domain, source, max_iterations=2000)[0]
...
def test_2d_low_contrast(n_domains):
    ...
    if n_domains is None:  # 1-domain, periodic boundaries (without wrapping correction)
        ...
        domain = Helmholtz(permittivity=n, periodic=periodic, pixel_size=pixel_size, wavelength=wavelength)
    else:  # OR. Domain decomposition
        periodic = np.where(np.array(n_domains) == 1, True, False)  # True for 1 domain in direction, False otherwise
        periodic = tuple(periodic)
        domain = MultiDomain(
            permittivity=n, periodic=periodic, wavelength=wavelength, pixel_size=pixel_size, n_domains=n_domains
        )

    u_computed = preconditioned_richardson(domain, source, max_iterations=10000)[0]
...
def test_2d_high_contrast(n_domains):
    ...
    u_computed = preconditioned_richardson(domain, source, max_iterations=int(1.0e5))[0]
    u_computed = u_computed.squeeze()[*([slice(boundary_widths, -boundary_widths)] * 2)]
...
def test_3d_disordered(n_domains):
    ...
    u_computed = preconditioned_richardson(domain, source, max_iterations=1000)[0]
...
def test_3d_homogeneous(n_domains):
    ...
    boundary_widths = 50
    # add boundary conditions and return permittivity (n²) and edge_widths in format (ax0, ax1, ax2)
    n, boundary_array = add_absorbing_boundaries(n, boundary_widths)
