...
if os.path.basename(os.getcwd()) == 'paper_code':
    ...
    filename = 'paper_figures/fig2_splitting.pdf'
else:
    try:
        os.makedirs('examples/paper_figures', exist_ok=True)
        filename = 'examples/paper_figures/fig2_splitting.pdf'
    except FileNotFoundError:
        filename = 'fig2_splitting.pdf'
...
domain = HelmholtzDomain(permittivity=n**2, periodic=periodic, 
...
boundary_widths = 100
n, boundary_array = preprocess((n**2).numpy(), boundary_widths)
assert n.imag.min() >= 0, 'Imaginary part of n is negative'
domain_o = HelmholtzDomain(permittivity=n, periodic=periodic, 
                           wavelength=wavelength, pixel_size=pixel_size, 
                           debug=True)
...
fig, axs = plt.subplots(1, 4, figsize=(12, 3), sharex=True, sharey=True, 
                        gridspec_kw={'wspace': 0.15, 'width_ratios': [1, 1, 1, 1.094]})
fraction = 0.046
pad = 0.04
